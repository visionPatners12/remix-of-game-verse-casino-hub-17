...
import { TopTipsters } from "../common/TopTipsters";
import { useInfiniteScroll } from '../../../hooks/useInfiniteScroll';
import { Loader2 } from 'lucide-react';
import type { BasePostProps } from '../../posts/base/BasePostProps';

/**
 * ForecastsView - Displays only prediction posts using local filtering
 * No additional API calls - filters from shared FeedDataContext
 */
export const ForecastsView = memo(function ForecastsView() {
  ...
  if (isLoading && filteredCount === 0) {
    return (
      <div className="space-y-6">
        <TopTipsters />
        <FeedSkeleton message={t('loading.predictions')} />
      </div>
    );
  }
        ...
        {filteredCount === 0 && !isLoading ? (
          <div className="p-6 text-center">
            <p className="text-muted-foreground">
              {t('empty.noPredictions')}
            </p>
          </div>
        ) : (
          <>
            {feed.map((item) => {
              // Only stream posts (predictions) will be in the filtered feed
