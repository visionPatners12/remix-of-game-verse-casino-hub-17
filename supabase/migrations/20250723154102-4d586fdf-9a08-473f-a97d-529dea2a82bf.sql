...
-- Index composé pour la recherche combinée
CREATE INDEX IF NOT EXISTS idx_users_full_search ON public.users (username, first_name, last_name);
...
BEGIN
  ...
  SELECT 
    ...
    GREATEST(
      ...
      COALESCE(similarity(u.first_name, search_term), 0),
      COALESCE(similarity(u.last_name, search_term), 0)
    ) as similarity_score
  FROM public.users u
  WHERE 
