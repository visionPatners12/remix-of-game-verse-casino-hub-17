-- 1) Supprimer TOUTES les versions de get_highlights
DROP FUNCTION IF EXISTS public.get_highlights(uuid, integer, timestamptz, uuid, uuid);
DROP FUNCTION IF EXISTS public.get_highlights(uuid, uuid, uuid, integer, text);

-- 2) Recréer avec les colonnes correctes (sans provider ni fixture_id)
