-- Migration: Tri des highlights par match_date au lieu de created_at
-- Drop et recréer la fonction pour ajouter le champ match_date
...
BEGIN
  ...
  SELECT 
    ...
    CASE WHEN m.id IS NOT NULL THEN
      jsonb_build_object('id', m.id, 'starts_at', m.starts_at, 'score', NULL::jsonb)
  ...
  WHERE 
    (p_cursor IS NULL OR h.match_date < p_cursor)
...
COMMENT ON FUNCTION public.get_highlights IS 'Récupère les highlights triés par match_date (matchs les plus récents en premier).';