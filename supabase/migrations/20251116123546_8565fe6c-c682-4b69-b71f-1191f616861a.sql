-- Fix: public.get_highlights — jointure via match_highlightly_id, fixture_id = m.id, score = NULL
...
BEGIN
  ...
  SELECT 
    ...
    h.home_team_id,
    h.away_team_id,
    m.id AS fixture_id, -- FIX: utiliser l'UUID du match (fixture)
    h.created_at,
    h.updated_at,
    COALESCE(
    ...
    CASE WHEN m.id IS NOT NULL THEN
      jsonb_build_object('id', m.id, 'starts_at', NULL, 'score', NULL::jsonb) -- FIX: score null et pas de référence à m.score
  ...
  LEFT JOIN sports_data.match m ON m.highlightly_id = h.match_highlightly_id -- FIX: jointure via match_highlightly_id
  WHERE 
...
GRANT EXECUTE ON FUNCTION public.get_highlights TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_highlights TO anon;

COMMENT ON FUNCTION public.get_highlights IS 'Récupère les highlights avec scoring de pertinence. Jointure match via highlightly_id, fixture_id = m.id, score null.';