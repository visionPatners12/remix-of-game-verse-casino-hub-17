-- Fix: Corriger le nom de table fixtures → match dans public.get_highlights
...
BEGIN
  ...
  LEFT JOIN sports_data.match m ON m.id = h.fixture_id
  WHERE 
...
COMMENT ON FUNCTION public.get_highlights IS 'Récupère les highlights avec scoring de pertinence. Utilise sports_data.match (et non fixtures).';