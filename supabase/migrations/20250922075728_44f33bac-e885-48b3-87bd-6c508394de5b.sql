-- Fix the ambiguous search_leagues function by dropping the version without sport_filter_id
-- and keeping only the more complete version

DROP FUNCTION IF EXISTS public.search_leagues(q text, n integer);

-- Ensure we have the correct function signature with sport_filter_id
CREATE OR REPLACE FUNCTION public.search_leagues(q text, n integer DEFAULT 10, sport_filter_id uuid DEFAULT NULL)
